...
HELLO_ENGINE_API_C UI_Municion* CreateUI_Municion(ScriptToInspectorInterface* script)
{
	...
	script->AddDragBoxGameObject("Cinematic", &classInstance->cinematic);
	script->AddDragBoxGameObject("Fade_to_black", &classInstance->fade_to_black_GO);
...
void UI_Municion::Start()
{
	...
	fadeToBlack = (FadeToBlack*)fade_to_black_GO.GetScript("FadeToBlack");
	if (!fadeToBlack) Console::Log("FadeToBlack Missing in UI_Municion.");
