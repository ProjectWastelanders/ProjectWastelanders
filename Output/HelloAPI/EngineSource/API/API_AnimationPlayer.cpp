...
void API::API_AnimationPlayer::SetAnimationSpeedMultiplier(float speed)
{
    if (!_animation)
    {
        Engine::Console::S_Log("Trying to acces a NULLPTR AnimationPlayer in SetAnimationSpeedMultiplier");
        return;
    }
    _animation->SetSpeedMultiplier(speed);
}
